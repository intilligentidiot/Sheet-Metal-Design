...
(function () {
    ...
    function drawBendDiagram(R, T, angleDeg, K) {
        ...
        svgContent += `
            <line x1="${cx}" y1="${cy}" x2="${rDimEnd.x.toFixed(1)}" y2="${rDimEnd.y.toFixed(1)}"
                  stroke="#cc0000" stroke-width="1.5" stroke-dasharray="4 3" opacity="0.6"/>
            <circle cx="${cx}" cy="${cy}" r="4" fill="#cc0000" opacity="0.6"/>`;

        // Labels — larger fonts
        svgContent += `
            <text x="${cx}" y="${cy - 12}" text-anchor="middle"
                  font-family="system-ui, sans-serif" font-size="16" font-weight="600" fill="#cc0000" opacity="0.9">
                R = ${R}
            </text>`;

        // Thickness label
        const tLabelPos = ptOnCircle(cx, cy, sOR + 40, startAngle);
        svgContent += `
            <text x="${tLabelPos.x}" y="${tLabelPos.y}" text-anchor="end"
                  font-family="system-ui, sans-serif" font-size="16" font-weight="600" fill="#ffffff" opacity="0.9">
                T = ${T}
            </text>`;
        ...
        svgContent += `
            <path d="M ${angStart.x.toFixed(1)} ${angStart.y.toFixed(1)}
                  ...
                  fill="none" stroke="#cc0000" stroke-width="2" stroke-dasharray="4 4" opacity="0.7"/>
        ...
        svgContent += `
            <g transform="translate(20, 370)">
                ...
                <line x1="0" y1="22" x2="22" y2="22" stroke="#cc0000" stroke-width="2.5" stroke-dasharray="6 4"/>
                <text x="28" y="27" font-family="system-ui, sans-serif" font-size="13" fill="#b0b0b0">Neutral Axis</text>
                <line x1="0" y1="44" x2="22" y2="44" stroke="#cc0000" stroke-width="2"/>
    ...
    dom.leadForm.addEventListener('submit', (e) => {
        ...
        if (email && emailRegex.test(email)) {
            ...
            dom.modalSuccess.querySelector('p').textContent = "Downloading your guide...";
            
            // Simulate download
            setTimeout(() => {
                // In a real app, trigger a PDF download here
                // window.open('guide.pdf', '_blank');
                dom.modalSuccess.querySelector('p').textContent = "Success! Check your downloads folder.";
            }, 1000);
    ...
    dom.printForm.addEventListener('submit', async (e) => {
        ...
        dom.printModalSuccess.querySelector('p').textContent = "Generating your report...";

        try {
            await downloadBendReportPDF(vals);
            dom.printModalSuccess.querySelector('p').textContent = "Success! Your report has been downloaded.";
            setTimeout(() => {
                closeModal();
            }, 2000);
        } catch (error) {
    ...
    // ──────── PDF GENERATION ────────
    async function downloadBendReportPDF(vals) {
        return new Promise((resolve, reject) => {
            ...
            document.getElementById('pdf-results').innerHTML = `
                <tr><td style="padding: 14px 16px; border-bottom: 1px solid #dcdcdc; font-weight: 700; color: #cc0000;">Bend Allowance (BA)</td><td style="padding: 14px 16px; border-bottom: 1px solid #dcdcdc; font-weight: 700; color: #cc0000; font-size: 16px;">${ba.toFixed(4)} ${currentUnit}</td></tr>
            ...
            const opt = {
                margin: 0,
                filename: 'Bend_Allowance_CNC_Report.pdf',
