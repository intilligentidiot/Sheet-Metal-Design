        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <article class="bg-darkBg border border-white/10 rounded-custom overflow-hidden group hover:border-primary/50 transition-colors duration-300 flex flex-col h-full">
                <div class="h-48 overflow-hidden relative">
                    <img src="<?= htmlspecialchars($post['image']) ?>" alt="<?= htmlspecialchars($post['title']) ?>" title="<?= htmlspecialchars($post['title']) ?>" loading="lazy" width="600" height="400" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                ...
                <div class="p-8 flex flex-col flex-grow">
                    ...
                    <h3 class="text-xl font-bold uppercase mb-4 group-hover:text-primary transition-colors leading-tight">
                        <a href="blog/<?= htmlspecialchars($post['slug']) ?>" title="Read Our Detailed Guide on <?= htmlspecialchars($post['title']) ?>"><?= htmlspecialchars($post['title']) ?></a>
                    </h3>
                    <p class="text-gray-400 font-light text-sm mb-6 flex-grow leading-relaxed">
                        <?= htmlspecialchars($post['excerpt']) ?>
                    </p>
                    <a href="blog/<?= htmlspecialchars($post['slug']) ?>" class="text-white hover:text-primary text-sm font-bold uppercase tracking-widest transition-colors mt-auto inline-flex items-center" title="Read the full article: <?= htmlspecialchars($post['title']) ?>">
